class StandingController < ApplicationController
  def show
    target_event = '1部リーグ戦'
    target_year = GameInfo.where(event: target_event).order(disp_date: :desc).first.disp_date.year
    target_season = '春季'
    @game_infos = GameInfo.where(event: target_event, season: target_season, gameset_flag: true).where('disp_date LIKE ?', "#{target_year}%")
    win_lose_draw_list = []
    teams = Team.where(league: target_event)
    teams.each do |team|
