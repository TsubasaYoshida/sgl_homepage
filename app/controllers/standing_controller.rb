class StandingController < ApplicationController
  ...
  def describe
    ...
    tmp_event_list.each_with_index do |event, i|
      ...
      if game_infos.size > 0
        ...
        # チーム情報、チーム成績を設定
        record_list = make_record_list(team_name_list, game_infos)

        # 順位決定ロジック
        decide_rank(record_list)

        # ◯×△（ハイフンと空文字も）の生成
        result_array = make_result_array(record_list, game_infos)
  ...
  def make_result_array(record_list, game_infos)
    result_array = []
